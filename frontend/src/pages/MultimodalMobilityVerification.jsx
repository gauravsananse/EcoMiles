...
export default function MultimodalMobilityVerification({ user, onUserUpdate, onOpenAuth }) {
  ...
  // Fetch initial transit context and beacons
  useEffect(() => {
    fetchTransitContext();
  }, []);
  ...
  // Live Sensor Polling & High-Frequency Stream (10 Hz)
  useEffect(() => {
  ...
  useEffect(() => {
    if (isTracking && !isPaused && isTestMode) {
      ...
      // Handle multi-leg auto progression
      let legTimeout;
      if (profile.isMultiLeg) {
  ...
  }, [isTracking, isPaused, isTestMode, selectedProfileKey, currentLegIndex]);
  ...
  const executeSlidingWindowInference = async () => {
    ...
    if (isTestMode) {
      ...
      sensorWindow = {
        ...
        sensorAvailability: { gps: true, accelerometer: true, gyroscope: true, bluetooth: true },
  ...
  const handleStartJourney = async () => {
    ...
    try {
      ...
      // 2. Initialize Journey session on backend
    ...
    <div className="main-content">
      ...
      <div className="page-header">
        <div className="badge-tag">
          ...
          <span>AI Multimodal Mobility Verification</span>
        </div>
        <h1 className="page-title">Continuous Transport Mode & Anti-Fraud Verification</h1>
        <p className="page-subtitle">
          Real multi-signal sensor fusion dynamically determines walking, cycling, bus, metro, scooter, or car travel, preventing fraudulent claims with cryptographic reward verification.
        </p>
      </div>
      ...
      <div className="card" style={{ maxWidth: '850px', margin: '0 auto 2rem' }}>
        ...
        <div style={{
          ...
          <div style={{ background: 'var(--slate-50)', borderRadius: '12px', padding: '0.85rem' }}>
            <div style={{ fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--slate-400)' }}>
              Duration
            </div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '1.6rem', fontWeight: 800, color: 'var(--slate-900)', marginTop: '0.2rem' }}>
          ...
          <div style={{ background: 'var(--slate-50)', borderRadius: '12px', padding: '0.85rem' }}>
            ...
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '1.6rem', fontWeight: 800, color: 'var(--primary-700)', marginTop: '0.2rem' }}>
              {totalDistanceKm.toFixed(2)} <span style={{ fontSize: '0.85rem', fontWeight: 600 }}>km</span>
            </div>
          </div>
          ...
          <div style={{ background: 'var(--slate-50)', borderRadius: '12px', padding: '0.85rem' }}>
            ...
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '1.6rem', fontWeight: 800, color: 'var(--slate-900)', marginTop: '0.2rem' }}>
              {liveSensorData.speed?.toFixed(1) || '0.0'} <span style={{ fontSize: '0.85rem', fontWeight: 600 }}>km/h</span>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center' }}>
          {!isTracking ? (
            <button
              ...
              style={{ width: '100%', maxWidth: '420px', padding: '0.85rem 1.5rem', fontSize: '1.05rem' }}
            >
      ...
      {/* Completion & Verification Success Banner */}
      {completionResult && (
        <div className="status-banner success" style={{ maxWidth: '850px', margin: '0 auto 2rem', animation: 'fadeIn 0.2s ease' }}>
          <CheckCircle2 size={30} className="text-emerald-600" style={{ flexShrink: 0, marginTop: '2px' }} />
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 800, fontSize: '1.2rem', marginBottom: '0.25rem' }}>
              🎉 Multimodal Journey AI Verified & Cryptographically Released!
            </div>
            <div style={{ fontSize: '0.88rem', marginBottom: '0.75rem' }}>
              <strong>{completionResult.segments?.length || 1} Travel Segments</strong> &bull; {completionResult.totalDistanceKm} km in {completionResult.totalDurationMinutes} mins
            </div>
            <div style={{
              display: 'flex',
              gap: '1rem',
